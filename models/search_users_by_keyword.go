// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/swag"
)

// SearchUsersByKeyword search users by keyword
// swagger:model search-users-by-keyword
type SearchUsersByKeyword struct {

	// users
	Users SearchUsersByKeywordUsers `json:"users"`
}

// Validate validates this search users by keyword
func (m *SearchUsersByKeyword) Validate(formats strfmt.Registry) error {
	var res []error

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// MarshalBinary interface implementation
func (m *SearchUsersByKeyword) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *SearchUsersByKeyword) UnmarshalBinary(b []byte) error {
	var res SearchUsersByKeyword
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
